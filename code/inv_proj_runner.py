...
class SimulationJob:
    """Resumable Monte Carlo job (GUI can cancel between batches via st.rerun)."""
    ...
    def run_batch(
        ...
        """Run up to ``batch_size`` more projections.

        Returns ``"running"`` if more remain, ``"done"`` when all finished.
        """
        if self._closed:
